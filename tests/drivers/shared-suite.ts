...
export interface SharedSuiteOpts {
  /** Name passed to getTables / getTableColumns / getTableForeignKeys etc. */
  db: string;
  /** Expected string inside getDatabases() results (case-insensitive). */
  expectInDatabases: string;
  /** Expected tables (lowercase). Defaults include the 5 seeded tables. */
...
/**
 * Common read-only tests exercising IDatabaseDriver against the seeded
 * arumu_test schema. Call inside a describe block — the driver must be
 * connected before the tests run (beforeAll in the caller).
 */
export function runSharedSuite(getDriver: () => IDatabaseDriver, opts: SharedSuiteOpts) {
  const tables = opts.expectedTables ?? DEFAULT_TABLES;

  // ── Schema discovery ──────────────────────────────────────────────────────

  describe('getDatabases', () => {
    it('returns array with expected entry', async () => {
  ...
  describe('getTables', () => {
    it('returns seeded tables', async () => {
  ...
  describe('getSchema', () => {
    it('returns map with column lists', async () => {
      ...
      // customers should have at least id, name, email
  ...
  describe('getTableColumns', () => {
    it('returns columns with correct structure for customers', async () => {
  ...
  describe('getTableIndexes', () => {
    it('returns PRIMARY and UNIQUE indexes for customers', async () => {
  ...
  describe('getTableForeignKeys', () => {
    it('returns FK from orders to customers', async () => {
  ...
  describe('getTableCreateStatement', () => {
    it('returns non-empty DDL string', async () => {
  ...
  describe('getTableData', () => {
    it('returns 10 rows with total=10 for customers', async () => {
    ...
    it('paginates: offset 5 returns 5 rows', async () => {
      const result = await getDriver().getTableData(opts.db, 'customers', 5, 5);
      expect(result.rows.length).toBe(5);
      expect(result.total).toBe(10);
    });

    it('sorts DESC by name', async () => {
  ...
  describe('getSupportedTypes', () => {
    it('returns non-empty type groups', async () => {
  ...
  describe('getCapabilities', () => {
    it('returns valid ServerCapabilities', () => {
  ...
  describe('escapeIdentifier / escapeStringLiteral', () => {
    it('escapeIdentifier wraps in appropriate quotes', () => {
    ...
    it('escapeStringLiteral escapes single quotes', () => {
      const escaped = getDriver().escapeStringLiteral("it's");
      expect(escaped).toContain("'");
      // The inner quote must be escaped somehow

...
import { diffSchema, hasChanges } from '../services/schemaDiff';
...
// Derived from the same diff the migration uses, so the Save button reflects
// whether there is actually a statement to run.
const hasPendingChanges = computed(() => hasChanges(getPendingChanges()));
...
/**
 * Asks main for the statements this migration would run. Generated by the
 * driver, so the preview matches the connected engine rather than always
 * rendering MySQL.
 */
const getAlterSql = async (): Promise<string | null> => {
  const statements = await api.previewSchemaChanges(
    props.serverName, props.database, props.table, getPendingChanges(),
  );
...
const handleCopyAlter = async () => {
  const alterSql = await getAlterSql();
  if (!alterSql) return;
...
const openAlterModal = async () => {
  const sql = await getAlterSql();
  if (!sql) return;

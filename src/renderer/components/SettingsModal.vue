...
<template>
  <div class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 backdrop-blur-sm px-4" @click.self="emit('close')">
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl w-full max-w-md overflow-hidden shadow-2xl">
      <div class="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
        ...
        <button @click="emit('close')" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-100 transition-colors p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded">
          <X :size="20" />
        </button>
      </div>
      ...
      <div class="p-6 space-y-6 text-slate-900 dark:text-slate-100">
        <div class="space-y-3">
          <label class="flex items-center gap-2 text-sm font-medium text-slate-600 dark:text-slate-300">
            <Languages :size="16" class="text-blue-600 dark:text-blue-400" />
            {{ $t('settings.language_label') }}
          </label>
          <div class="relative">
            <select 
              v-model="language" 
              class="w-full appearance-none bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 pr-10 text-slate-900 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all cursor-pointer"
            >
        ...
        <div class="space-y-3">
          <label class="flex items-center gap-2 text-sm font-medium text-slate-600 dark:text-slate-300">
            <Moon :size="16" class="text-blue-600 dark:text-blue-400" />
